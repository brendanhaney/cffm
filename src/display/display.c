  ...
  switch(key) {
    ...
    case key_down:
      ...
      if (state.visual_mode) 
        handle_input(key_space, dis, dir, NULL);

      if (dir->selected < dir->folderc+dir->filec-1) 
        dir->selected++;
...
char *get_selection_buffer() {
  ...
  if (state.copying) 
    strncat(buffer, "cp -r ", 7);
  else if (state.cutting) 
    strncat(buffer, "mv ", 4);
  else if (ALLOW_DELETE && state.deleting) 
    strncat(buffer, "rm -r ", 7);
  else 
    strncat(buffer, "echo ", 6); /*undefined, so harmless command? :)*/
...
void get_updates(Display *dis) {
  ...
  if (dis->width != w.ws_col || dis->height != w.ws_row) {
    ...
    if (state.show_border) 
      dis->height = w.ws_row -1;
    else 
      dis->height = w.ws_row - abs(1-state.show_border);
